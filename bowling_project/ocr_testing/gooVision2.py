...
def get_document_bounds(image_file, feature):
    ...
    # Example usage
    search_word = 'Game'
    bounding_box = find_word_bounding_box(response, search_word)
    if bounding_box:
        print(f"Bounding box for '{search_word}': {bounding_box}")
    else:
...
def find_word_bounding_box(response, search_word):
    """Finds the bounding box of the first occurrence of search_word in the OCR response."""
        ...
        for block in page.blocks:
            for paragraph in block.paragraphs:
                for word in paragraph.words:
                    ...
                    if word_text.lower() == search_word.lower():
                        # Found the word, return its bounding box
...
render_doc_text('croptest.jpg', 'out.jpg')
"""
